  ...
  getFileAsText(container = undefined, fileToGet = undefined, callback) {
    this.blobService.getBlobToText(container, fileToGet, (error, result, response) => {
      if (error) {
        ...
        logger.info({
          msg: "Azure__StorageService : getFileAsText success response for container " + container + " for file " + fileToGet + " response " + response,
        });
      }
    });
  }
      ...
      try {
        ...
        form.on("part", (part) => {
          if (part.filename) {
            ...
            logger.info({
              msg: "Azure__StorageService : blockStreamUpload Uploading file to container " + uploadContainer + " to folder " + blobFolderName + " for file name " + name + " with size " + size,
            });
  ...
  upload(container, fileName, filePath, options = {}, callback) {
    this.blobService.createBlockBlobFromLocalFile(container, fileName, filePath, options, (error, result, response) => {
      if (error) {
        ...
        callback(null, null, response);
        logger.info({
          msg: "Azure__StorageService : upload success response for container " + container + " for file " + filename + " response " + response,
        });
      }
    });
  }
