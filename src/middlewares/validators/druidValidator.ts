...
const validate = (configParams: Record<string, any>) => (req: Request, res: Response, next: NextFunction) => {
    ...
    if (isSqlQuery) {
        ...
        result = ValidationService.validateSqlQuery(queryPayload, getLimits(dataSource))
    }

    else {
        dataSource = queryPayload.dataSource
        result = ValidationService.validateNativeQuery(queryPayload, getLimits(dataSource))
    }
