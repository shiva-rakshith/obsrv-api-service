class testDruidQuery{
    ...
    public static HIGH_DATE_RANGE_GIVEN_AS_LIST='{"queryType":"groupBy","dataSource":"telemetry-events","dimensions":["actor_type","content_framework"],"limit":15, "metric":"count","granularity":"all","intervals":["2021-01-02/2021-02-05"],"aggregations":[{"type":"count","name":"count"}]}';
    public static HIGH_DATE_RANGE_GIVEN_AS_STRING='{"queryType":"topN","dataSource":"telemetry-events","dimension":"actor_id","threshold":10,"metric":"count","granularity":"all","intervals":"2020-12-30/2021-02-02","aggregations":[{"type":"count","name":"count"}]}';
    ...
    public static VALID_SQL_QUERY="{\"query\":\"SELECT * FROM \\\"telemetry-events\\\" WHERE __time >= TIMESTAMP '2020-12-31' AND __time < TIMESTAMP '2021-01-21' LIMIT 10\"}";
    public static HIGH_LIMIT_SQL_QUERY="{\"query\":\"SELECT mid FROM \\\"telemetry-events\\\" WHERE __time >= TIMESTAMP '2021-01-01' AND __time < TIMESTAMP '2021-01-22' LIMIT 100\"}";
    ...
    public static HIGH_DATE_RANGE_SQL_QUERY="{\"query\":\"SELECT actor_type, content_status FROM \\\"telemetry-events\\\" WHERE __time >= TIMESTAMP '2021-01-01' AND __time < TIMESTAMP '2021-02-12' LIMIT 10\"}";
